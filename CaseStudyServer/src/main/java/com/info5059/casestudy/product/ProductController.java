...
public class ProductController {
    ...
    public ResponseEntity<Integer> deleteOne(@PathVariable String id) { // string??
        return new ResponseEntity<Integer>(productRepository.deleteOne(id), HttpStatus.OK);
    }

    @GetMapping("/api/products/{id}")
    public ResponseEntity<Iterable<Product>> findByEmployee(@PathVariable Integer id) { // use RequestBody here
        return new ResponseEntity<Iterable<Product>>(productRepository.findByVendorid(id),
                HttpStatus.OK);
    }
